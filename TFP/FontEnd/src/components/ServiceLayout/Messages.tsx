  ...
  const createThread = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/communications/threads`, {
        ...
        body: JSON.stringify({ channel, subject: composeSubject, participants: parts })
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: 'Erreur' }));
            ...
            {showCompose && (
              <div className="p-4 mb-4 bg-neutral-900/40 rounded">
                ...
                <div className="flex gap-2 mb-2">
                  <select onChange={e => setComposeParticipants(prev => prev ? prev + ', ' + e.target.value : e.target.value)} className="bg-neutral-800 border border-neutral-700 text-neutral-200 rounded px-3 py-2">
                    <option value="">Ajouter un employé...</option>
                    {employees.map(emp => (
                      <option key={emp.id} value={`employee:${emp.id}`}>{emp.name} ({emp.type})</option>
                    ))}
                  </select>
                  <button onClick={() => { setComposeParticipants(''); setComposeSubject(''); setShowCompose(false); }} className="btn-secondary">Annuler</button>
                  <button onClick={createThread} className="btn-primary">Créer</button>
                </div>
              </div>
            )}
            ...
            <div className="card-elevated">
              <div className="mb-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 ${getTypeInfo(selectedMessage.type).bg} rounded-full flex items-center justify-center`}>
                      <span className="text-2xl">{getTypeInfo(selectedMessage.type).icon}</span>
                    </div>
                    <div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button onClick={sendReply} className="btn-primary flex-1">
                    <svg className="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
                    </svg>
                    Envoyer la réponse
                  </button>
                  <button onClick={() => setReply('')} className="btn-secondary">
                    Effacer
                  </button>
                </div>
              </div>
