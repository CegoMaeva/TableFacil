...
def require_manager_auth(f):
    """Middleware pour vérifier que l'utilisateur est un gérant"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            # Vérifier que c'est un employé de type gérant
            if payload.get('user_type') != 'employee' or payload.get('role') != 'gerant':
                return jsonify({'error': 'Accès réservé aux gérants'}), 403
            
            # Ajouter les données du gérant à la requête
