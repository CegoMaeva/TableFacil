...
export const AssistantClient: React.FC = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      // Extraire la réponse enrichie ou simple texte
      let responseText = '';
      let richData: RichAnswer | undefined;
      
      if (typeof data.answer === 'object' && data.answer.text) {
          ...
          <div className="mt-4 pt-4 border-t border-neutral-700 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3">
              {richData.recipes.map((recipe: Recipe) => (
                <div
                  key={recipe.id}
                  className="flex gap-3 p-3 bg-neutral-900/40 rounded-lg border border-neutral-700 hover:border-emerald-500/50 transition cursor-pointer"
        ...
        >
          ...
          {messages.map((m, idx) => (
            <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] px-4 py-3 rounded-xl border text-sm ${
                m.role === 'user'
