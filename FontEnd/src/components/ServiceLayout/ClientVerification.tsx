...
export const ClientVerification = () => {
  ...
  const loadAllClients = async () => {
    ...
    try {
      ...
      // Normaliser + dédoublonner (clé email > téléphone > nom)
      const dedup = new Map<string, any>();

      const upsert = (entry: any) => {
        const key = normalize(entry.email) || normalize(entry.phone) || normalize(entry.name);
        if (!key) return;
        const existing = dedup.get(key);
        if (!existing) {
          dedup.set(key, { ...entry, reservationsCount: entry.kind === 'reservation' ? 1 : 0, kinds: new Set([entry.kind]) });
        } else {
          existing.reservationsCount = (existing.reservationsCount || 0) + (entry.kind === 'reservation' ? 1 : 0);
          ...
          existing.raw = existing.raw || entry.raw;
        }
      };
      ...
      const flattened = Array.from(dedup.values()).map((r) => ({
        ...r,
        kinds: Array.from(r.kinds || [])
      }));

      flattened.sort((a, b) => normalize(a.name).localeCompare(normalize(b.name)));
    ...
    try {
      ...
      if (resResponse.ok) {
        ...
        const found = reservations.find((r: any) => 
          normalize(r.customer).includes(normalize(searchTerm)) ||
          normalize(r.email).includes(normalize(searchTerm)) ||
          normalize(r.phone).includes(normalize(searchTerm))
        );
      ...
      if (usersResponse.ok) {
        ...
        const foundUser = users.find((u: any) => 
          ...
          normalize(u.email).includes(normalize(searchTerm)) ||
          normalize(u.phone).includes(normalize(searchTerm))
        );
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Debug info supprimée pour épurer l'affichage */}
          ...
          {results.map((r) => (
            ...
            >
              <div>
                ...
                <div className="text-neutral-400 text-sm">{r.email || r.phone || 'Aucun contact'}</div>
                {r.reservationsCount > 1 && (
                  <div className="text-xs text-amber-300 mt-1">{r.reservationsCount} réservations liées</div>
                )}
              </div>
              <div className="text-neutral-400 text-sm text-right">
                {r.kind === 'reservation' ? r.date : 'Client'}
                {r.kinds?.length > 1 && <div className="text-xs text-emerald-300">Multi-source</div>}
              </div>
            </div>
          ))}
        </div>
